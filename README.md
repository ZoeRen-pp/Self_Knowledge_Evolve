...
- **声明式查询引擎**：5 种代数原语（seed/expand/combine/aggregate/project），关系代数+图遍历混合执行，自动依赖分析与并行调度
- **Knowledge Copilot**：自然语言提问 → 自动术语提取 → 构建查询计划 → 执行 → LLM 组织回答
- **Dashboard**：6 Tab 可视化面板（系统总览、知识探索、质量评估、知识演化、运行监控、Copilot），支持离线导出为单文件 HTML
...
```
                    ┌─────────────────────────────────────┐
                    │           FastAPI (src/app.py)       │
                    │  查询引擎 + 21算子 + Copilot + Dashboard │
                    └──────────────┬──────────────────────┘
                                   │
                    ┌──────────────▼──────────────────────┐
                    │     Query Engine (src/query/)        │
                    │  seed → expand → combine → aggregate │
                    │  → project  (声明式查询计划)         │
                    └──────────────┬──────────────────────┘
                                   │
              ┌────────────────────┼────────────────────┐
...
`POST /api/v1/query` — 客户端提交 JSON 查询计划，引擎自动验证、依赖分析、分波并行执行。

| 原语 | 对应代数 | 功能 |
|------|----------|------|
| `seed` | σ (Selection) | 从 ID/别名/层/Embedding/属性初始化结果集 |
| `expand` | ⋈+TC (Join+Transitive Closure) | 图遍历（本体关系+保留边 tagged_in/rst_adjacent/evidenced_by） |
| `combine` | ∪∩− (Set Ops) | 并集/交集/差集 |
| `aggregate` | γ (Aggregation) | count/rank/group/score/rerank（含关键词相关性+可选 cross-encoder） |
| `project` | π (Projection) | 字段投影 |
...
`POST /api/v1/copilot` — 自然语言提问，自动检索知识并组织回答。

```
用户问题
  ↓ LLM 提取搜索关键词（中英文技术术语）
  ↓ 模糊搜索（alias 精确匹配 → 子串匹配 → embedding 语义匹配）
  ↓ 匹配到本体节点 ID + 置信度
  ↓ LLM 根据匹配结果动态生成查询计划（验证失败自动重试，最多 3 次）
  ↓ 查询引擎执行
  ↓ Facts 按五层分组（scenario → condition → method → mechanism → concept）
  ↓ LLM 按五层模型组织回答
```
...
```
├── src/
│   ├── query/                 # 声明式查询引擎（engine/planner/executors/validator/reranker/types）
│   ├── api/query_router.py    # POST /api/v1/query
│   ├── api/copilot_router.py  # POST /api/v1/copilot（自然语言知识问答）
│   ├── api/semantic/          # 21 语义算子 API
│   ├── api/system/            # 系统管理 API（监控、审核、showcase）
...
├── scripts/                   # 运维脚本
├── worker.py                  # Worker（4线程，Pipeline持续填充模式）
└── docs/                      # 设计文档
```
...
- `docs/architecture-design-20260406.md` — 系统架构设计
- `docs/development-spec-20260406.md` — 开发规格说明
- `docs/query-engine-design-20260417.md` — 声明式查询引擎设计

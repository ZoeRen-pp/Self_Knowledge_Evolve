...
```
┌─────────────────────────────────────────────────────────────────┐
│  表现层   FastAPI REST + Dashboard + Copilot                     │
├─────────────────────────────────────────────────────────────────┤
│  查询层   QueryEngine 5原语: seed/expand/combine/aggregate/project│
│           Planner(依赖分析→分波并行) + Validator + Reranker      │
│           Copilot(NL→LLM提词→模糊搜索→LLM规划→执行→五层回答)    │
├─────────────────────────────────────────────────────────────────┤
...
### 7.4 Stage 4 的五级优先抽取架构

Stage 4 使用五级优先策略，从结构推断到统计兜底逐级降低：

**P0 RST 结构推断**（无 LLM 调用）：利用 Stage 2 的篇章关系直接推导 fact。5 种 RST 类型映射为谓语：Constraint→constrained_by, Prerequisite→requires, Causation→triggers, Condition→applies_when, Exemplification→has_example。如果两个相邻段落各自只对齐到 1 个本体节点，且 RST 关系在上述 5 种之内，直接生成 fact，无需 LLM。

**P1 单段 LLM 抽取**（核心路径）：要求段落至少有 3 个锚点节点（来自 Stage 3 的 canonical + mechanism_tag + method_tag + condition_tag + scenario_tag），发送给 LLM 抽取 (S,P,O) 三元组。新增**引用验证（quote grounding）**：LLM 返回的每个 triple 需要在原文中找到支持证据（精确子串匹配 OR ≥80% token overlap），拦截幻觉。

**P2 合并上下文 LLM**：只对特定篇章关系类型合并——Elaboration、Sequence、Causation、Evidence、Background、Exemplification 六种"连续型"关系表明相邻段落在讲述同一件事，合并后信息更完整。Contrast/Constraint/Condition/Prerequisite 等合并后反而引入混淆。合并时在 prompt 头部注入 RST nuclearity 提示。

**P2-chain 多跳 RST 链**：沿 Sequence/Elaboration/Causation 关系寻找 3-4 段连续链（≤1500 tokens），用 `[Paragraph N]` 标记每段，单次 LLM 调用提取跨段 fact。

**P3 双节点共现兜底**：仅当段落恰好有 2 个 canonical 节点且匹配到谓语信号模式时触发。语义价值低，但从 S/A 级来源的共现 confidence 可达 0.74。
...
### 10.1 为什么篇章关系使用闭合词表（10 种）

LLM 在没有约束时会发明谓语：`is_affected_by`、`impacts_performance_of`、`can_be_configured_via`... 谓语词表开放会导致 Neo4j 里出现数百种关系类型，图遍历变得不可控。

**为什么从 RST 原版 21 种缩减到 10 种段落级类型**：标准 RST 的粒度是子句（clause）级，在这个粒度上 Elaboration / Explanation / Restatement / Summary 是可区分的四种关系。但本系统的切分单元是段落（100–1024 tokens），一个段落内部通常同时包含多个子句级关系，在段落粒度强行区分这四种会导致 LLM 分类随机漂移。

10 种类型的设计标准：**在段落粒度有明确的语言学信号可以区分**。每种类型都对应一套 discourse marker（"for example"→Exemplification、"however"→Contrast、"MUST"→Constraint...），LLM 按信号分类稳定性高、可解释性强。

本系统借鉴 RST 的"话语单元之间存在修辞功能关系"这一框架，但在段落粒度重新定义了类型边界，是一套领域适配的段落级篇章关系词表，不是标准 RST 的直接使用。
...
### 11.2 三级回退：HTTP BGE-M3 → Ollama → sentence-transformers

`src/utils/embedding.py` 在启动时自动探测可用后端，结果缓存：

```
优先级 1: HTTP BGE-M3 服务（EMBEDDING_HTTP_URL, 默认 :8000）
          → WSL2 systemd 运行的 BGE-M3 HTTP 服务，延迟最低
          → POST /embedding {"text": "..."} → 1024-dim float vector
优先级 2: Ollama API（OLLAMA_URL, 默认 :11434）
          → 不在 Python 进程内加载模型，内存开销小
          → /api/embed 接口，支持批量（每批 64 文本）
优先级 3: sentence-transformers（本地 Python）
          → model.encode()，首次加载慢，适合 CI/CD
```

三者输出格式完全相同（1024 维 float list），上层代码不感知差异。端口 `:8000` 被 BGE-M3 服务占用，因此应用使用 `:8001`（通过 `.env` 的 `APP_PORT` 配置）。
...
│                       60+ 种子 URL（10 站点），TLS 指纹/robots.txt 尊重
│                       重试退避：5min → 30min → 120min，最多 3 次
├── _pipeline_thread  — 持续处理 raw 文档，走 7 阶段 pipeline
│                       LLM 硬依赖：不可用时阻塞等待（每 120s 重试 ping）
│                       4 并发 worker 线程，持续填充模式（非批次等待）
│                       每个 worker 完成一篇即取下一篇，大文档不阻塞其他 worker
├── _stats_thread     — 每 5 分钟采集系统指标快照
└── _maintenance_thread — 每天 03:00 CST 执行本体维护
                          三阶段：embedding 去重 → LLM 分类 → 清理合并
                          持久化状态到 logs/maintenance_state.json
```
...
**_pipeline_thread 对 LLM 的硬要求**：每批文档处理前用 `llm.ping(timeout=10)` 检查 LLM 可达性。如果不可达，整个 pipeline 线程暂停（每 120s 重试），直到 LLM 恢复。这保证了不会用降级模式（纯共现关系）产出低质量数据。

**LLM 熔断器（Circuit Breaker）**：`LLMExtractor` 内置三连败熔断。连续 3 次网络级失败（ConnectTimeout/ConnectError/5xx）触发熔断，此后所有调用 `is_enabled()` 的线程进入阻塞等待（每 30s ping LLM + 调用 keep-alive 回调保持 DB 连接），直到 LLM 响应恢复。HTTP 4xx（除 429）不算作网络故障——服务器正常工作，是请求本身有问题。429 使用指数退避重试。

**种子源管理**：`_SEED_SOURCES` 定义 10 个站点（RFC Editor S 级、Huawei/Juniper/Arista A 级、FRRouting/NetworkLessons/Cloudflare/PacketLife/ipSpace/Noction B 级），合计 60+ 种子 URL，启动时自动入库（ON CONFLICT DO UPDATE 幂等）。
...
- `context_signals.yaml`：上下文信号（用于 Stage 3 语义角色检测）
- `predicate_signals.yaml`：谓语信号（用于 Stage 4 P3 共现关系谓语选择）
- `candidate_stopwords.yaml`：候选词停用词表（用于 Stage 3 候选词过滤）
...
| LLM 接入 | OpenAI 兼容 httpx（默认 Gemini） | LangChain / LlamaIndex | 轻量；切换厂商只改 base_url |
| Embedding | HTTP BGE-M3 → Ollama → sentence-transformers 三级回退 | OpenAI embedding API | 本地推理零成本；无 API 依赖 |
| LLM 可用性 | 硬要求（阻塞等待恢复 + DB keep-alive） | 降级到共现兜底 | 知识质量不能为可用性妥协 |
| 冲突检测 | 本体驱动（cardinality=one 才触发 D4） | 全谓语冲突检测 | 多值谓语不是矛盾，避免假阳性 |

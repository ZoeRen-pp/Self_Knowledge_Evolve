...
**为什么只对特定篇章关系类型合并**：只有 Elaboration、Sequence、Causation、Evidence、Background、Exemplification 这六种"连续型"关系才触发合并——因为这六种关系表明相邻段落在讲述同一件事，合并后信息更完整。Contrast（两段讲不同选项）、Constraint（一段限制另一段）、Condition/Prerequisite（条件关系）等合并后反而引入混淆。这个判断依赖 Stage 2 计算的篇章关系结果（`t_rst_relation.relation_type`）。

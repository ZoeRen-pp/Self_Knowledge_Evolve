...
当运维人员面对故障告警时，他们需要跨越这道语言鸿沟，在脑子里手动完成术语归一化，才能把从不同来源获取的知识拼在一起。这是一个认知负担极重、且非常容易出错的过程。
...
这四个问题对应四个独立的认知框架，对不同角色有不同价值：运维人员关心场景和条件，协议工程师关心机制，架构师关心概念关系。扁平本体把这四类知识混在同一个层面，导致查询粒度无法控制。
...
五层来自对电信运维知识结构的实际分析：一个有经验的工程师在处理故障时，脑子里同时维护这五类知识，缺少任何一类都会导致诊断不完整。

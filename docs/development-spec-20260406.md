...
**职责**：将 clean_text 切分为语义自洽的段落单元（每段只承担一种 communicative role），并计算 SimHash 和段落间的篇章关系。

**核心设计原则**：切分边界 = 语义角色变化边界。段落的类型（segment_type）和切分边界通过同一次 LLM 分类决定，不再是先切后标。

**四步联合切分流程**：

```
1. structural_split   — 按标题/章节标记切出 section 级 chunk（保留 section_path）
       ↓
2. split_into_paragraphs — 每个 chunk 按 \n\n 切成段落级候选单元
       ↓
3. classify_paragraphs — 对全文所有段落做一次 LLM batch 分类，每段分配 segment_type
       ↓
4a. 丢弃 noise 段落（TOC、版权声明、导航条等）
4b. merge_same_type — 同 section_path、同 segment_type 的相邻段落合并（合并后 ≤ 1024 tokens）
4c. apply_length_control — 合并后仍超长的段落做三级切分（段落→句子→滑动窗口）
```

**结构切分规则（自动检测格式）**：

```python
if 检测到 markdown heading (# ## ###):
    _split_markdown(text)   # 按 # 级别维护 section_path 层次
elif 检测到 RFC 数字编号节 or 全大写标题:
    _split_rfc(text)        # "1.  Introduction" 格式
else:
    _split_plaintext(text)  # 按三连空行或 form-feed 切
```

**17 种 segment_type（由 LLM 在段落粒度分类）**：

| 组 | segment_type | 含义 |
|---|---|---|
| 知识本质层 | definition | 概念/术语/协议字段/数据格式定义 |
| | mechanism | 协议/算法/进程如何在内部工作 |
| | scenario | 部署拓扑、业务用例、网络设计场景 |
| 操作层 | config | 配置语法、参数说明、单步命令 |
| | procedure | 含验证/回滚的有序多步骤工作流 |
| | prerequisite | 任务/配置开始前必须满足的前提条件 |
| 条件层 | constraint | 协议内部 MUST/SHALL 规范性规则、限制 |
| | compatibility | 跨厂商/版本行为差异、互操作性事实 |
| 评估建议层 | best_practice | 设计建议、安全加固、反模式 |
| | performance | 指标、容量规划、定时器值、吞吐量基准 |
| | comparison | 技术/选项权衡分析，辅助决策 |
| | fault | 故障症状、错误状态、调测失败现象 |
| | troubleshooting | 诊断步骤、验证过程、调试命令 |
| 结构层 | table | 有意义的数据表格（参数参考、特性矩阵） |
| | code | CLI 输出、配置片段、代码块 |
| 过滤层 | noise | TOC、样板文字、导航栏 → 被丢弃 |
| | unknown | 实质性内容但无法归入以上类型 |

**段落合并规则**：

同时满足以下全部条件才合并相邻两段：
- `segment_type` 相同（且不为 `unknown`，避免 LLM 关闭时过度合并）
- `section_path` 相同（不跨标题合并）
- 合并后 token 数 ≤ 1024

**SimHash** 计算在最终 segment 存储时对 raw_text 做词频加权 64 位哈希，供 Stage 5 Hamming 距离近重复检测。

**篇章关系**（见第 9 节）：在最终 segment 列表上，对相邻段落对做 LLM 分类，存入 `t_rst_relation`，包含 `relation_type` 和 `nuclearity`（NS/SN/NN）。

**输出**：`segments` 表行，每行含 `segment_id`、`source_doc_id`、`segment_index`、`segment_type`、`raw_text`、`normalized_text`、`simhash_value`、`token_count`、`confidence`
...
Stage 2 在最终 segment 列表上，对相邻段落对做 LLM 分类，结果存入 `t_rst_relation` 表（`relation_type` + `nuclearity`），Stage 4 用来判断是否合并相邻段落重试抽取。

**设计原则**：借鉴 RST（修辞结构理论）的"话语单元之间存在修辞功能关系"这一框架，但在段落粒度重新定义类型——保留在段落粒度有语言学信号可区分的关系，合并在句子级才能区分的细粒度类型（如 Explanation/Restatement/Summary 全部归入 Elaboration）。

| 组 | 关系类型 | 描述 | Nuclearity |
|---|---|---|---|
| 语义扩展 | Elaboration | B 在 A 的主题上补充更多内容（最通用的扩展关系） | NS |
| | Exemplification | B 给出 A 的具体例子、场景或应用实例 | NS |
| 逻辑结构 | Sequence | B 在时序/步骤上紧接 A（过程类文档的核心关系） | NN |
| | Causation | A 导致 B，或 B 解释 A 成立的原因 | NS |
| | Contrast | B 提供与 A 不同的选项、行为或立场 | NN |
| 规范约束 | Constraint | B 对 A 的行为施加规范性限制（MUST/SHALL/MUST NOT） | NS |
| | Condition | A 是条件，B 是条件满足时的内容（if A then B） | SN |
| | Prerequisite | 理解或执行 B 需要先完成 A；B 是主任务 | SN |
| 支撑上下文 | Evidence | B 为 A 的主张提供数据、引用或规范依据 | NS |
| | Background | A 为理解 B 提供必要的背景/动机/历史上下文 | SN |

**Nuclearity 说明**：
- `NS`（Nucleus-Satellite）：A 是主体（核），B 是支撑（卫星）
- `SN`（Satellite-Nucleus）：B 是主体（核），A 是铺垫/条件（卫星）
- `NN`（Multinuclear）：A 和 B 地位平等，均为核

**Stage 4 中的应用逻辑**：

以下关系触发"合并上下文重试"（把相邻两段合并成一个更大的上下文窗口再送 LLM 抽取）：

| 触发合并 | 不触发合并 |
|---|---|
| Elaboration、Sequence、Causation、Evidence、Background、Exemplification | Contrast、Constraint、Condition、Prerequisite |

触发合并的关系说明两段在讲述同一件事，合并后 LLM 能看到更完整的信息。不触发的关系说明两段讨论的是不同侧面或施加限制，合并后会混淆抽取目标。
